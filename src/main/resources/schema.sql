...
CREATE TABLE IF NOT EXISTS users (
  ...
  login varchar(50) NOT NULL, CONSTRAINT login_without_spaces CHECK (locate(login,' ') = 0),
  email varchar(50) NOT NULL, CONSTRAINT email_with_at CHECK (length(email) > 0 AND (locate('@',email) != 0)),
  birthday date NOT NULL, CONSTRAINT birthday_in_past_or_present CHECK (birthday <= now())
);
